...
public class CyanGhost extends Ghost
{
    ...
    public CyanGhost(float x, float y) {
        super(x, y, 36, 36);
        allImages = new TextureRegion[][]{
            {
                new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostRIGHT0.png"))),
                new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostRIGHT1.png"))),
                new TextureRegion(new Texture(Gdx.files.internal("assets/EyesRIGHT.png")))
            },
            {new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostLEFT0.png"))),
                new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostLEFT1.png"))),
                new TextureRegion(new Texture(Gdx.files.internal("assets/EyesLEFT.png")))
            },
            {new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostUP0.png"))),
                new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostUP1.png"))),
                new TextureRegion(new Texture(Gdx.files.internal("assets/EyesUP.png")))
            },
            {new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostDOWN0.png"))),
                new TextureRegion(new Texture(Gdx.files.internal("assets/CyanGhostDOWN1.png"))),
    ...
    public void move(BackgroundCell[][] map, Cell pacTarget, Cell redTarget, Move facing) {
        if (getStatus() == Status.ENTERING) {
            ...
            } else if (getY() >= 34 + (22 * 31) - (22 * 13) + 15) {
    ...
    public Cell adjustedTarget(Cell pac, Cell red, Move f) {
        Cell t;
        if (f == Move.UP)
            t = new Cell(pac.getX() + (pac.getX() - red.getX()), (pac.getY() - 2) + ((pac.getY() - 2) - red.getY()));
        else if (f == Move.DOWN)
            t = new Cell(pac.getX() + (pac.getX() - red.getX()), (pac.getY() + 2) + ((pac.getY() + 2) - red.getY()));
        else if (f == Move.LEFT)
            t = new Cell((pac.getX() - 2) + ((pac.getX() - 2) - red.getX()), pac.getY() + (pac.getY() - red.getY()));
        else if (f == Move.RIGHT)
            t = new Cell((pac.getX() + 2) + ((pac.getX() + 2) - red.getX()), pac.getY() + (pac.getY() - red.getY()));
        else
            t = new Cell(pac.getX() + (pac.getX() - red.getX()), pac.getY() + (pac.getY() - red.getY()));
